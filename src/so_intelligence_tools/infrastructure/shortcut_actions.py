...
from so_intelligence_tools.infrastructure.runtime import LinuxRuntime
...
def build_default_shortcut_registry(runtime: LinuxRuntime) -> ShortcutActionRegistry:
    registry = ShortcutActionRegistry()
    registry.register(
        "selected-text-correction",
        lambda: run_selected_text_correction(runtime),
    )
    return registry
