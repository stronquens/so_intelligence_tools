## MODIFIED Requirements

### Requirement: Linux Whisper backend bootstrap
The system SHALL prepare the faster-whisper Docker backend when installing Linux push-to-talk dictation integration and after Linux user login.

#### Scenario: Linux desktop integration installs dictation
- **WHEN** the user runs the Linux desktop integration installer
- **THEN** the system SHALL ensure `docker/whisper-server/.env` exists
- **AND** the system SHALL run the faster-whisper Docker Compose service before enabling the dictation listener service.

#### Scenario: User installs only the dictation service
- **WHEN** the user runs the standalone push-to-talk dictation service installer
- **THEN** the system SHALL ensure the faster-whisper Docker backend is started before enabling the service.

#### Scenario: Linux user logs in after reboot
- **WHEN** the Linux user session starts after reboot
- **THEN** the system SHALL run a user-level voice-runtime startup service
- **AND** that service SHALL ensure the faster-whisper Docker backend before the dictation listener accepts shortcuts.

#### Scenario: Whisper image supports autogenerated authentication
- **WHEN** the project creates or repairs the default Whisper Docker environment
- **THEN** it SHALL explicitly configure the localhost-only Whisper server without an autogenerated API key
- **AND** the default dictation client SHALL reach the server without manual token recovery.
