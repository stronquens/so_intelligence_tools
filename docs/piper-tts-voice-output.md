...
Piper is selected automatically on Linux with:
...
The resolved endpoint is `http://127.0.0.1:9010`. Setting
`LOCAL_TTS_BACKEND=chatterbox` switches this host to the optional GPU backend;
it does not alter the Windows default.
...
To start both Linux voice runtimes used by dictation and Codex audio:
...
Task boundary speech is stateful. Repeated start events from the Codex app-server are deduplicated, tool/function/command events inside a turn do not end the task, and "Fin de tarea" is spoken only after a real terminal turn event. When a turn completes in full-cycle modes, pending queued speech is cleared and "Fin de tarea" is spoken next. Speech already playing may finish first, but old queued paragraphs are skipped.
...
On Linux, `install-linux-desktop-integration` enables `so-intelligence-tools-voice-runtimes.service`, which starts Piper and Whisper after login. If audio is silent after reboot, check:
...
If Docker Desktop was selected before reboot and local Docker services are missing, recover with:
