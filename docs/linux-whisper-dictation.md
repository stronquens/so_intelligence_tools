...
That installer also enables `so-intelligence-tools-voice-runtimes.service`, a
user-level oneshot service that starts Whisper plus the selected local TTS
backend after login. Linux `auto` selects Piper; Chatterbox requires an explicit
GPU opt-in.
