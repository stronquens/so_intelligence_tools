...
def test_virtual_microphone_loads_null_sink_and_unloads_module(monkeypatch):
    ...
    assert "sink_name=so_ai_test_mic" in run_calls[0]
    assert "rate=24000" in run_calls[0]
    assert popen_calls[0][:4] == ["pacat", "--device", "so_ai_test_mic", "--format=s16le"]
